...
struct NewsFeedView: View {
    ...
    func fetchArticlesForSelectedTopics() {
        ...
        for topic in selectedTopics {
            guard let url = URL(string: "\(baseURL)\(topic)_parsed.json") else {
                print("Invalid URL for topic: \(topic)")
                continue
            }
            ...
            URLSession.shared.dataTask(with: url) { data, _, error in
                ...
                if let error = error {
                    print("Error fetching \(topic): \(error.localizedDescription)")
                    return
                }
                
                guard let data = data else {
                    print("No data for topic \(topic)")
                    return
                }
                ...
                do {
                    ...
                    print("JSON decode error for topic \(topic): \(error)")
                }
            }.resume()
        }
